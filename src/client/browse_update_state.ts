...
export function pageVersion(
  ...
  const raw = doc.documentElement.getAttribute(`data-mokabook-${kind}-version`);
...
export function navigationPending(doc: Document): boolean {
  return doc.documentElement.hasAttribute("data-mokabook-navigating");
}

export function beginNavigation(doc: Document): void {
  doc.documentElement.setAttribute("data-mokabook-navigating", "");
}

export function finishNavigation(doc: Document): void {
  doc.documentElement.removeAttribute("data-mokabook-navigating");
  const Event = doc.defaultView?.Event;
  if (Event) doc.dispatchEvent(new Event("mokabook:navigation-settled"));
}
...
export async function waitForNavigation(
  ...
  while (navigationPending(doc) && !signal.aborted) {
    await new Promise<void>((resolve) => {
      const settled = () => {
        doc.removeEventListener("mokabook:navigation-settled", settled);
        signal.removeEventListener("abort", settled);
        resolve();
      };
      doc.addEventListener("mokabook:navigation-settled", settled, {
        once: true,
      });
