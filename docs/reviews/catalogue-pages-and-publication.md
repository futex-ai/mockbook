...
these new items remain open for user selection.

## Review After 3eae8be

1. **Medium — imported asset bytes escape authoring-input tracking. Open.**
   [Graph classification](../../src/build/source_inventory.ts) excludes loaders
   ...
   `ignore`, and remained public. This violates the mixed-role source rule and
   leaves authored output stale during development. Inventory freshness checks
   ...
   rebuilding authored output. **Recommended: A**, using the shared graph boundary
...
2. **Medium — Changes misses edits behind a stable public alias. Open.**
   [Resource matching](../../src/server/changed_resources.ts) compares logical
   reference routes to Git paths, while the watcher also records physical paths.
   ...
   action was `reload`, but `changedRoutes` was empty. Users can miss a real
   ...
   serve, watch, and publication. **Recommended: A**, with target-only edit tests
...
3. **Medium — publication follows unconfined links and drops valid public aliases. Open.**
   [Fingerprint enumeration](../../scripts/preview/inputs.mjs) follows unrelated
   symlinks when hashing, while [public copying](../../scripts/preview/catalogue.mjs)
   skips them before applying its public-file guard. Probes confirmed a read of
   ...
   **Recommended: A**, plus exported-reference checks in both modes and coverage
...
4. **Low — CLI and nested-entry documentation remains contradictory. Open.**
   The [package contract](../protocol/mokabook-package.md) rejects `review` and
   `--out` but still describes a Review CLI output override. Parser probes reject
   both `review` and `serve --out`, as the existing CLI tests require. The
   [NestedChild comment](../../src/authoring/types.ts) also names only screens and
   ...
   **Recommended: A.** Existing parser tests cover runtime; a blanket flag-string
...
check; it did not rerun write-producing test suites. The completed full gate
above supplies runtime verification. Independent probe scripts/results are
...
with boundary regressions is the recommended next scope; no new plan is created
and completed feature milestones remain complete.
