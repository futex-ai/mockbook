...
Planning complete; implementation has not started. The plan remains active.
...
- [Page compatibility and migration](../docs/protocol/mokabook-page-migration.md)
  owns explicit source registration, historical readers, the five Accounting
...
Keep complete-document rendering and generic source adapters. Add `definePage`
and nested `page`; normalize explicit `legacy.entries` registrations into the
same entry kind. Write one v4 manifest without `legacyPages`. Current serving
requires rebuilt v4 output; v2/v3 remain supported at the historical comparison
boundary. Whole-document comparisons and page-valued use-case steps are outside
this change; pages use Current and participate in Changes as specified.
...
adapters before switching consumers to v4; do not activate a breaking cutover
while any active compiler, server, publisher, or fixture still expects only v3.
Temporary implementation adapters are internal and removed at the final
cutover; no environment or migration flags appear in product views.
...
The implementation delivery gate remains Milestone 7.
...
snapshot. Planning commit/push and its post-push review follow this validated
snapshot; their results are reported in the delivery response.

## Milestone 2: Record unified page designs
...
## Milestone 3: Implement page definitions and source migration
...
- [ ] Extend config validation and `src/legacy/pages.ts` with exact-source
      registrations. Normalize pages before shared registry checks; test missing,
      excluded, unknown, unsafe, and duplicate registrations and source aliases.
- [ ] Add tests before changing rendering: synchronous complete HTML, one
      render/output, raw HTML components, legacy lint policies, exception and
...
- [ ] Prepare common catalogue indexes and route/artifact lookups for pages;
      ...
      Keep transition wiring usable until all runtime consumers are switched.
...
## Milestone 4: Complete runtime links, impact, and publication
...
- [ ] Update watcher inputs, reload attribution, and published catalogue
      ...
      states, and unchanged transactional/screen-comparison behavior.
...
## Milestone 5: Use one hierarchy throughout Browse
...
## Milestone 6: Finish consumer cutover and compatibility proof
...
- [ ] Register the already-designed synthetic document as a page in the basic
      example and add explicit registrations to the Accounting packed fixture.
      Cover `.source.html`
      components, aliases, and old-source preservation without product-specific
      code in Mokabook. Update ESM/NodeNext/npx/Juno checks for the new API/schema.
- [ ] Switch remaining current readers and outputs to v4, require explicit
      source registrations, and remove temporary runtime adapters. Keep historical
      v2/v3 readers at the comparison boundary and retain their regression fixtures.
...
- [ ] Build and pack the candidate. In a disposable Accounting checkout,
      register the five IDs and owning collections from the migration protocol,
      update exact-inventory/hierarchy/source-policy tests and docs, and regenerate
      output. Do not alter the synced inspection workspace.
...
## Milestone 7: Verify, commit, push, and review
...
registrations, collection memberships, source-policy updates, docs, and
